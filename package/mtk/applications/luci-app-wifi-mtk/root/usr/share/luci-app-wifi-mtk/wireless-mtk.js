		...
		s.addModalOptions = function(s) {
			return network.getWifiNetwork(s.section).then(function(radioNet) {
				...
				if (hwtype == 'mtk') {
					...
					o = ss.taboption('general', form.Flag, 'vendor_vht', _('Enable 256-QAM'), _('802.11n 2.4Ghz Only'));
					o.depends({'_freq': '2g', '!contains': true});
					o.default = o.enabled;
				...
				if (hwtype == 'mtk') {
					...
					o = ss.taboption('general', form.ListValue, 'mapmode', _('Mesh Mode'));
					o.depends('mode', 'mesh');
					o.value('0', _('Disable'));
					o.value('1', _('Map Turnkey'));
					o.value('2', _('BS 2.0'));
					...
					o = ss.taboption('advanced', form.Value, 'macaddr', _('MAC address'), _('Override default MAC address - the range of usable addresses might be limited by the driver')/*, _('WFA 802.11v Spec - If one of the Dot11vMbssid is enabled the whole MBSSID will follow the 802.11v Spec')*/);
				...
				if (hwtype == 'mtk') {
					...
					if (hwtype == 'mtk') {
						...
						o.defaults = {
							...
							'1': [{ encryption: 'sae-mixed' }, { encryption: 'wpa3-mixed' }],
							'0': []
						};
