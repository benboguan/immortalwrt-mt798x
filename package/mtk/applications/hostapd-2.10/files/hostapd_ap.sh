...
{
	echo " "
	echo "USAGE: hostapd_ap.sh [ra0|rai0|rax0|both] [security option: open|wpapsk|wpa2psk|wpawpa2mixedpsk|wepopen|wepshare|EnterpriseAP|wpsopen|wpswpa2psk|owe|sae|suiteB|wpa3mixedpsk] [optional: conf=<conf_file_path>]"
...
if [ "$1" = "rai0" ]; then
	...
	if [ "$2" = "open" ]; then
		...
		echo "rax0(5G) security is wpa3mixedpsk."
		/usr/bin/hostapd -B /etc/hostapd_rax0_wpa3mixedpsk.conf
	else 
		echo "AP has no this security($2)"	
	fi	
