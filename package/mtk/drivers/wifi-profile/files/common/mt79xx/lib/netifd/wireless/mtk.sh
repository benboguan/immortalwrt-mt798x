...
mtk_sta_vif_pre_config() {
	...
	json_get_vars disabled band encryption key key1 key2 key3 key4 ssid mode bssid wps_pushbutton pin pbc macaddr \
		...
		uapsd mumimo_ul ofdma_dl ofdma_ul ocv mwds
	json_select ..
...
mtk_mesh_vif_pre_config() {
	...
	[ "$encryption" == "wep-open" -o "$encryption" == "wep-shared" ] && MeshWEPKEY="${key1}" || MeshWPAKEY="${key}"
...
is_ax4200_dev() {
	grep -q "mt7986-ax4200" /etc/wireless/l1profile.dat 2>/dev/null && echo yes
}

is_ax6000_dev() {
	grep -q "mt7986-ax6000" /etc/wireless/l1profile.dat 2>/dev/null && echo yes
}
...
drv_mtk_setup() {
	...
	case "$phy_name" in
		ra0)
			...
			MTWIFI_IFPREFIX=""
			MTWIFI_DEF_BAND="2g"
			if is_ax4200_dev; then
				base="mt7986-ax4200.dbdc.b0"
			elif is_ax6000_dev; then
				base="mt7986-ax6000.dbdc.b0"
				RddAntSel=0 ; HT_RxStream=4 ; HT_TxStream=4
			else
				base="mt7981.dbdc.b0"
			fi
			;;
		rax0)
			...
			MTWIFI_DEF_BAND="5g"
			if is_ax4200_dev; then
				base="mt7986-ax4200.dbdc.b1"
				HT_RxStream=3 ; HT_TxStream=3
			elif is_ax6000_dev; then
				base="mt7986-ax6000.dbdc.b1"
				RddAntSel=2 ; HT_RxStream=4 ; HT_TxStream=4
			else
				base="mt7981.dbdc.b1"
			fi
			;;
		*) echo "Unknown phy:$phy_name"; return 1 ;;
	esac
	MTWIFI_PROFILE_PATH="${MTWIFI_PROFILE_DIR}${base}.dat"
	MTWIFI_CMD_PATH="${MTWIFI_PROFILE_DIR}${base}.cmd.sh"
	MTWIFI_CMD_OPATH="${MTWIFI_PROFILE_DIR}${base}.cmd_other.sh"

#检查配置文件目录是否存在，否则创建目录
	[ ! -d $MTWIFI_PROFILE_DIR ] && mkdir -p "$MTWIFI_PROFILE_DIR"
	echo > "$MTWIFI_CMD_PATH"
	...
	case "$band" in
		...
		*)
			...
			band=${MTWIFI_DEF_BAND:-2g}; band_idx=0 ;;
	esac
...
#For DBDC profile merging......
	...
	sed -i "s/BssidNum=.*/BssidNum=${BssidNum}/" "$MTWIFI_PROFILE_PATH"
	{
		echo "ApMWDS=${ApMWDS%?}"
...
#For WDS profile merging......
	...
	sed -i "s/WdsNum=.*/WdsNum=${WdsNum}/" "$MTWIFI_PROFILE_PATH"
	{
...
#停用wapp
	...
	if mtk_try_lock; then
		...
		# 删除旧的合并配置文件，防止残留重复配置
		rm -f "$MTWIFI_PROFILE_DIR/DBDC_card0.dat" 2>/dev/null
