...
sed -i "s/map_agent_alid=.*/map_agent_alid=${agent_al_mac}/g" /etc/map/1905d.cfg
    ...
    ra0_798x="$(uci get wireless.ra0.bandsteering)"
    if [ "$ra0_798x" -eq "1" ]; then
	ra0=1
	rax0=1
    fi
    
    ra0_798x="$(uci get wireless.ra0.ieee80211r)"
    if [ "$ra0_798x" -eq "1" ]; then
	ra0=1
    fi
    
    ra0_798x="$(uci get wireless.default_rax0.steeringthresold)"
    if [ "$ra0_798x" -lt "0" ]; then
	ra0=1
    fi
        
    ra0_798x="$(uci get wireless.default_ra0.disabled)"
    if [ "$ra0_798x" -eq "1" ]; then
	ra0=0
	rax0=0
    fi
    
    rax0_798x="$(uci get wireless.rax0.ieee80211r)"
    if [ "$rax0_798x" -eq "1" ]; then
	rax0=1
    fi
   
    rax0_798x="$(uci get  wireless.default_rax0.steeringthresold)"
    if [ "$rax0_798x" -lt "0" ]; then
	rax0=1
    fi
    
    rax0_798x="$(uci get wireless.default_rax0.disabled)"
    if [ "$rax0_798x" -eq "1" ]; then
	rax0=0
    fi
...
rax0_798x="$(uci get wireless.default_rax0.steeringbssid)"
if [ $rax0_798x ]; then
	bash setbssid rax0 "$rax0_798x"
fi

ra0_798x="$(uci get wireless.default_ra0.steeringbssid)"
