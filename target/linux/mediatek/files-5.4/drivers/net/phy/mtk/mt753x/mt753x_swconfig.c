...
void gpy211_set_port_link(struct switch_dev *dev, int port, struct switch_port_link *link)
{
	struct gsw_mt753x *gsw = container_of(dev, struct gsw_mt753x, swdev);
	u32 reg_val = 0;
	...
	case AN_DEFAULT:
		...
		zld_mii_write(gsw, port, MDIO_DEV0, MII_BMCR, reg_val);
		break;
	case SPEED1000_FULL:
	case SPEED2500_FULL:
...
static const struct switch_dev_ops mt753x_swdev_ops = {
	...
	.set_port_powerdown = mt753x_set_port_powerdown,
	.set_port_powerup = mt753x_set_port_powerup,
